...
export function findHardestSolutions(
  cards: Card[],
  ...
  cards.forEach((c) => {
    if (c.cardType == "number" || c.cardType == "alternate") {
      if (c.value >= 100) {
        collectedSolutionMap.set(c.value, {
          stepsTaken: [],
  ...
  const hardestAnswers = solutionsList.filter(
    (s) => s.difficultyEstimate > minDiff,
  );
    ...
    for (let j = i + 1; j < root.cards.length; j++) {
      ...
      for (const sol of nextSteps) {
        ...
        for (const s of solutionValues) {
          ...
          if (
            !currentMapVal ||
            currentMapVal.difficultyEstimate > sol.difficultyEstimate
          ) {
...
function getNextPossibleSteps(
      ...
      return [
        {
          ...
          difficultyEstimate:
            root.difficultyEstimate +
            difficultyOfStep(step, root.stepsTaken, res),
  ...
  const nextSteps: SolutionStep[] = [
    {
      stepType: "binary",
      operation: "+",
      left: arg1 as PlugCard,
      right: arg2 as PlugCard,
    },
    {
      stepType: "binary",
      operation: "-",
    ...
    },
    {
      stepType: "binary",
      operation: "-",
      left: arg2 as PlugCard,
      right: arg1 as PlugCard,
    },
    {
      stepType: "binary",
      operation: "*",
  ...
  const nextEvaluations: [SolutionStep, Card[]][] = nextSteps
    .map((ns) => {
  ...
  switch (step.stepType) {
    ...
    case "binary": {
      ...
      if (step.operation == "+" || step.operation == "-") {
        ...
        if (
          easyMult.includes(step.left.value) ||
          easyMult.includes(step.right.value)
        ) {
          diff = 0.7;
        } else {
          diff = 1;
      ...
      if (
        !(
          step.left.cardType == "number" &&
          step.left.modifier.modifierType == "none"
        )
      ) {
        return diff;
      }
